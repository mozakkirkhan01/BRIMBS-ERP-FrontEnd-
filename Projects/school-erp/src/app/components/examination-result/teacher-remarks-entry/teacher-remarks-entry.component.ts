...
@Component({
  selector: 'app-teacher-remarks-entry',
  templateUrl: './teacher-remarks-entry.component.html',
  styleUrls: ['./teacher-remarks-entry.component.css']
})
export class TeacherRemarksEntryComponent {
  FilterModel: any = {
    isChangeAll: false,
    Term: 0
  };
  ...
  getEntryTeacherRemarkList() {
    ...
    this.dataLoading = true
    this.service.getEntryTeacherRemarkList(request).subscribe(r1 => {
      let response = r1 as any
      if (response.Message == ConstantData.SuccessMessage) {
  ...
  changeHyTeacherRemarks(index: number, Marksheet: any) {
    if (index == 0 && this.FilterModel.isChangeAll) {
      this.MarksheetList.forEach(x1 => x1.HyTeacherRemarks = Marksheet.HyTeacherRemarks);
    }
  }

  changeFinalTeacherRemarks(index: number, Marksheet: any) {
    if (index == 0 && this.FilterModel.isChangeAll) {
      this.MarksheetList.forEach(x1 => x1.FinalTeacherRemarks = Marksheet.FinalTeacherRemarks);
    }
  }
  ...
  saveTeacherRemark() {
    ...
    this.MarksheetList.forEach(x1 => MarksheetList.push({
      ...
      HyTeacherRemarks:x1.HyTeacherRemarks,
      FinalTeacherRemarks:x1.FinalTeacherRemarks,
    }))
    var obj = {
    ...
    this.dataLoading = true;
    this.service.saveTeacherRemark(request).subscribe(r1 => {
      let response = r1 as any
      if (response.Message == ConstantData.SuccessMessage) {
        this.toastr.success("Teacher remarks details submitted successfully.");
        this.MarksheetList = [];
      } else {
        this.toastr.error(response.Message)
      }
