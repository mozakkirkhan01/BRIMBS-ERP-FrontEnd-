...
@Component({
  selector: 'app-product-issue-list',
  templateUrl: './product-issue-list.component.html',
  styleUrls: ['./product-issue-list.component.css']
})
export class ProductIssueListComponent {
  ...
  editIssue(obj: any) {
    this.router.navigate([`/admin/product-issue/${this.loadData.replaceSpecialCharacter(this.localService.encrypt(obj.IssueId))}`])
  }
  ...
  exportToExcel() {
    ...
    setTimeout(() => {
      this.loadData.exportToExcel(this.table_1, "Issue List " + this.loadData.loadDateTime(new Date()));
  ...
  deleteIssue(obj: any) {
    if (confirm("Are your sure you want to delete this recored")) {
      ...
      this.dataLoading = true
      this.stockService.deleteIssue(request).subscribe(r1 => {
        let response = r1 as any
        if (response.Message == ConstantData.SuccessMessage) {
