...
export class ManufacturerComponent {
  ...
  Manufacturer: ManufacturerModel = {} as ManufacturerModel
  ...
  resetForm() {
    this.Manufacturer = { Status:Status.Active} as ManufacturerModel;
  ...
  saveManufacturer() {
    this.formManufacturer.control.markAllAsTouched();
    if (this.formManufacturer.invalid) {
      this.toastr.error(ConstantData.formInvalidMessage)
      return
    }
    if (this.Manufacturer.ManufacturerId)
      this.Manufacturer.UpdatedBy = this.staffLogin.StaffLoginId;
    else
      this.Manufacturer.CreatedBy = this.staffLogin.StaffLoginId;
    var obj: RequestModel = {
      request: this.localService.encrypt(JSON.stringify(this.Manufacturer)).toString()
    }
    this.dataLoading = true;
    this.stockService.saveManufacturer(obj).subscribe(r1 => {
      let response = r1 as any
      if (response.Message == ConstantData.SuccessMessage) {
  ...
  deleteManufacturer(obj: any) {
    if (confirm(ConstantData.deleteConfirmation)) {
      ...
      this.dataLoading = true
      this.stockService.deleteManufacturer(request).subscribe(r1 => {
        let response = r1 as any
        if (response.Message == ConstantData.SuccessMessage) {
          this.toastr.success(ConstantData.deleteMessage);
          this.ManufacturerList = this.ManufacturerList.filter(x => x.ManufacturerId != obj.ManufacturerId);
        } else {
          this.toastr.error(response.Message)
        }
        this.dataLoading = false
