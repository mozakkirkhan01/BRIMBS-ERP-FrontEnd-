...
export class StaffComponent {
  ...
  StaffPhoto:string|null = null;
  Signature:string|null = null;
  imageUrl = ConstantData.getBaseUrl();
  setStaffPhotoFile(event: any) {
    var file: File = event.target.files[0];
    if (file.type != 'image/jpeg' && file.type != 'image/jpg' && file.type != 'image/png') {
      this.toastr.error("Invalid file format !!");
      this.Staff.StaffPhotoFile = null;
      this.Staff.StaffPhoto = '';
      this.StaffPhoto =this.imageUrl+ this.Staff.StaffPhoto;
      return;
    }
    if (file.size < 512000) {
      ...
      reader.addEventListener('load', (e1: any) => {
        ...
        this.Staff.StaffPhoto = base64Data;
        this.StaffPhoto =`data:image/png;base64,${base64Data}` ;
      });
  ...
  setSignatureFile(event: any) {
    var file: File = event.target.files[0];
    if (file.type != 'image/jpeg' && file.type != 'image/jpg' && file.type != 'image/png') {
      this.toastr.error("Invalid file format !!");
      this.Staff.SignatureFile = null;
      this.Staff.Signature = '';
      this.Signature =this.imageUrl+ this.Staff.Signature;
      return;
    }
    if (file.size < 512000) {
      var reader = new FileReader();
      ...
      reader.addEventListener('load', (e1: any) => {
        var dataUrl = e1.target.result;
        var base64Data: string = dataUrl.substr(dataUrl.indexOf('base64,') + 'base64,'.length);
        this.Staff.Signature = base64Data;
        this.Signature =`data:image/jpeg;base64,${base64Data}` ;
      });
